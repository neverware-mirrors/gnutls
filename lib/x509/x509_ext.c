				...
				struct ct_sct_st *sct)
{
	...
	/* TODO check that there are actually no extensions */
	ptr += 2;
	length -= 2;
...
int gnutls_x509_ext_ct_import_scts(const gnutls_datum_t *ext, gnutls_x509_ct_scts_t scts)
{
	...
	ASN1_TYPE el = ASN1_TYPE_EMPTY;

	retval = asn1_create_element(_gnutls_get_pkix(),
				     "PKIX1.SignedCertificateTimestamps", &el);
	if (retval != ASN1_SUCCESS) {
		gnutls_assert();
		return _gnutls_asn2err(retval);
	}

	retval = _asn1_strict_der_decode(&el, ext->data, ext->size, NULL);
	if (retval != ASN1_SUCCESS) {
		gnutls_assert();
		asn1_delete_structure(&el);
		return _gnutls_asn2err(retval);
	}

	retval = _gnutls_x509_read_value(el, "", &scts_content);
	if (retval < 0) {
		gnutls_assert();
		asn1_delete_structure(&el);
		return _gnutls_asn2err(retval);
	...
	/* _gnutls_x509_read_value allocs new memory, so we can safely get rid of 'el' here */
	asn1_delete_structure(&el);
...
int gnutls_x509_ext_ct_export_scts(const gnutls_x509_ct_scts_t scts, gnutls_datum_t *ext)
{
	...
	for (unsigned i = 0; i < scts->size; i++) {
		...
		if (written < 0) {
			gnutls_assert();  /* XXX: gnutls_assert_val() ??? */
			gnutls_free(out);
			return -1;
		}
		offset += written;
	...
	/* TODO offset and ttl_data should be equal at this point - should we check that? */

	/* DER-encode the whole thing as an opaque OCTET STRING, as the spec mandates */
	ret =
		asn1_write_value(el, "", out, ttl_size+2);  /* again, leave extra room for 2 length bytes */
	if (ret < 0) {
		gnutls_assert();
		ret = _gnutls_asn2err(ret);
		goto cleanup;
	...
	ret = _gnutls_x509_der_encode(el, "", ext, 0);
	if (ret < 0) {
		gnutls_assert();
		goto cleanup;
	}
